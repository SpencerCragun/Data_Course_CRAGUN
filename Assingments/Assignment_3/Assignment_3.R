...
dat[row,col] <- "41.000" # This changes any 41mm found to 41.000 

levels(dat$Headwidth)
as.numeric(dat$Headwidth)
headwidth2 <- as.numeric(as.character(dat$Headwidth)) #### COnverted to character first, then to numeric using the as. functions

dat$Headwidth <- headwidth2
...
#### Open the connection, give it the plot, then run the connection ####
jpeg("./testimage.jpg")
plot(nums, main = "My Title", xlab = "My axis label", ylab = "My other axis label")
dev.off()
...
jpeg("../Data_Course_CRAGUN/Assingments/Assingment_3//testimage.jpg")
plot(x=dat$Headwidth,y=dat$Mass, main = "Thatch Ant Head Size vs. Mass", xlab = "Headwidth", ylab = "Mass")
dev.off()

# 3.  Subset the thatch ant data set to only include ants from colony 1 and colony 2
Colony1 <- dat[which(dat$Colony == 1), c(1:7)]
Colony2 <- dat[which(dat$Colony == 2), c(1:7)]
Colony1_2 <- rbind(Colony1, Colony2)

# 4.  Write code to save this new subset as a .csv file
write.csv(Colony1_2, "../Data_Course_CRAGUN/Assingments/Assingment_3/Colony1_2")
